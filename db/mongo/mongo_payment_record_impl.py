...
class MongoPaymentRecordImpl():
    ...
    # 转账记录表名
    def payment_record_tbl(self) -> None|Collection:
        if self.mongo_impl.db is None:
            self.logger.error("MongoDB connection is not established.")
            return None
        return self.mongo_impl.db['payment_record_tbl']
    ...
    return {*}
    """
    def query_by_id(self, id: str) -> tuple[bool, PaymentRecordDao|None]:
        tbl_name = self.payment_record_tbl()
        if tbl_name is None:
